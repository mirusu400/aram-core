...
func lessEqualPushes(depth int, top int16) []byte {
	var code []byte
	for i := 0; i < depth; i++ {
...
func lessEqualPrime(t *testing.T, v *gvm.VM, depth int) {
	t.Helper()
...
func lessEqualFault(t *testing.T, v *gvm.VM, cause error, offset int) {
	t.Helper()
...
func TestBranchLessEqualSignedMatrix(t *testing.T) {
		...
		for _, immediate := range []int8{-128, -1, 0, 1, 126, 127} {
			for _, depth := range []int{1, 65} {
				t.Run(fmt.Sprintf("top%d/imm%d/depth%d", top, immediate, depth), func(t *testing.T) {
					code := lessEqualPushes(depth, top)
					offset := len(code)
					target := offset + 5
					code = append(code, 0x3e, byte(immediate), byte(target>>8), byte(target), 0xff, 0xff)
					v := gvm.New(code)
					lessEqualPrime(t, v, depth)
...
func TestBranchLessEqualHostFaultOrder(t *testing.T) {
		...
		for _, top := range []int16{-1, 0, 1} {
			for n := 0; n < 3; n++ {
				t.Run(fmt.Sprintf("trunc%d/depth%d/top%d", n, depth, top), func(t *testing.T) {
					...
					v := gvm.New(code)
					lessEqualPrime(t, v, depth)
					lessEqualFault(t, v, gvm.ErrTruncated, offset)
				})
			}
		}
	}
	for _, target := range []int{4, 5, 65535} {
		v := gvm.New([]byte{0x3e, 0, byte(target >> 8), byte(target)})
		lessEqualFault(t, v, gvm.ErrStackUnderflow, 0)
	}
	for _, depth := range []int{1, 65} {
		for _, top := range []int16{-1, 0} {
			for _, extra := range []int{0, 1, 65535} {
				t.Run(fmt.Sprintf("invalid/depth%d/top%d/extra%d", depth, top, extra), func(t *testing.T) {
					...
					code = append(code, 0x3e, 0, byte(target>>8), byte(target))
					v := gvm.New(code)
					lessEqualPrime(t, v, depth)
					lessEqualFault(t, v, gvm.ErrInvalidTarget, offset)
				})
			}
		}
	}
...
func TestBranchLessEqualUnusedTargetsEndFetch(t *testing.T) {
	for _, depth := range []int{1, 65} {
		for _, extra := range []int{0, 1, 65535} {
			code := lessEqualPushes(depth, 1)
			offset := len(code)
			target := offset + 4 + extra
			if extra == 65535 {
				target = 65535
			...
			lessEqualFault(t, v, gvm.ErrTruncated, len(code))
		}
	}
}

func TestBranchLessEqualBudget(t *testing.T) {
	for _, top := range []byte{0, 1} {
		v := gvm.New([]byte{5, top, 0x3e, 0, 0, 7, 0xff, 0xff})
		lessEqualPrime(t, v, 1)
		stack := v.Stack()
...
func TestBranchLessEqualMemoryAndCallReturnPreserved(t *testing.T) {
	for _, top := range []byte{0, 1} {
		// Two nested calls establish distinct saved PCs. Both paths return twice.
		code := []byte{0x44, 0, 6, 5, 42, 0xff, 0x44, 0, 10, 0x45, 5, top, 0x3e, 0, 0, 17, 0x45, 0x45}
