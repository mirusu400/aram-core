...
func TestBranchNotEqualPublicPushMatrix(t *testing.T) {
	for imm := 0; imm < 256; imm++ {
		...
		word := uint16(s & 65535)
		for _, top := range []uint16{word, word ^ 1, uint16(imm), 0x8000, 0x7fff} {
			for _, depth := range []int{1, 64, 65} {
				code := make([]byte, 0, depth*3+6)
				for i := 0; i < depth; i++ {
					w := uint16(0xa000 + i)
					if i == depth-1 {
						w = top
				...
				fallthroughPC := len(code) + 4
				target := fallthroughPC + 1
				code = append(code, 0x40, byte(imm), byte(target>>8), byte(target), 0xff, 0xff)
				v := New(code)
				clear(code)
				if err := v.Run(uint64(depth)); err != ErrBudget || v.depth != depth {
					t.Fatalf("prime depth=%d: %v", depth, err)
				}
				before := *v
				before.depth--
				before.stack[before.depth] = 0
				before.pc = fallthroughPC
				if top != word {
					before.pc = target
				}
				owned := v.Stack()
...
func notEqualVM(code []byte, depth int, top uint16) *VM {
	v := New(code)
	v.depth = depth
	for i := 0; i < depth; i++ {
...
func notEqualFault(t *testing.T, v *VM, cause error) {
	t.Helper()
	before := *v
	program := v.Program()
	err := v.Step()
...
func TestBranchNotEqualImmediateMatrix(t *testing.T) {
	for imm := 0; imm < 256; imm++ {
		...
		for _, top := range tops {
			for _, depth := range []int{1, 64, 65} {
				v := notEqualVM([]byte{0x40, byte(imm), 0, 5, 0xff, 0xff}, depth, top)
				before := *v
				before.depth--
				before.stack[before.depth] = 0
				before.pc = 4
...
func TestBranchNotEqualFramingAndTargets(t *testing.T) {
		...
		for _, top := range []uint16{0x80, 0xff80} {
			for tail := 0; tail < 3; tail++ {
				t.Run(fmt.Sprintf("framing%d/depth%d/top%x", tail, depth, top), func(t *testing.T) {
					v := notEqualVM([]byte{0x40, 0x80, 0xff, 0xff}[:1+tail], depth, top)
					notEqualFault(t, v, ErrTruncated)
				})
			}
			for _, target := range []int{0, 3, 4, 65535} {
				t.Run(fmt.Sprintf("target%d/depth%d/top%x", target, depth, top), func(t *testing.T) {
					v := notEqualVM([]byte{0x40, 0x80, byte(target >> 8), byte(target)}, depth, top)
					if depth == 0 {
						notEqualFault(t, v, ErrStackUnderflow)
						return
					}
					if top != 0xff80 && target >= 4 {
						notEqualFault(t, v, ErrInvalidTarget)
						return
					}
...
func TestBranchNotEqualNewAtAbsolute(t *testing.T) {
	...
	for _, target := range []int{0, 1, 0x1234, 0x8001, 0xffff} {
		...
		code := make([]byte, size)
		copy(code[7:], []byte{5, 0x7f, 0x40, 0x80, byte(target >> 8), byte(target)})
		code[target] = 0xff
		v, err := NewAt(code, 7)
		if err != nil {
			t.Fatal(err)
...
func TestBranchNotEqualBudgetAndHalt(t *testing.T) {
	for _, top := range []byte{0, 1} {
		v := New([]byte{5, top, 0x40, 0, 0, 7, 0xff, 0xff})
...
func TestBranchNotEqualConstructorsMemoryAndNestedReturns(t *testing.T) {
	for _, mode := range []string{"new", "at", "symbols", "address", "services"} {
		for _, top := range []byte{0, 1} {
			t.Run(fmt.Sprintf("%s/top%d", mode, top), func(t *testing.T) {
				...
				code := []byte{0x44, 0, 6, 5, 42, 0xff, 0x44, 0, 10, 0x45, 5, top, 0x0b, 0x40, 0, 0, 18, 0x45, 0x45}
...
func TestBranchNotEqualSavedStateAndMemoryFaultAtomicity(t *testing.T) {
	for _, valid := range []bool{false, true} {
		for _, tail := range []int{0, 1, 2, 3} {
			code := []byte{0x40, 0x80, 0xff, 0xff}[:1+tail]
		...
		for _, top := range []uint16{0, 1} {
			v := notEqualVM([]byte{0x40, 0, 0, 0}, 1, top)
...
func TestBranchNotEqualLiveProgramOperands(t *testing.T) {
	...
	offset := uint32(7)
	v, err := NewWithSymbols([]byte{0x36, 0, 0x80, 6, 0, 0x80, 0x40, 0, 0, 11, 0xff, 0xff}, 0, []SymbolRegion{{ProgramOffset: &offset, Length: 2}})
	if err != nil {
		t.Fatal(err)
	}
	// Store writes ff80 LE, so the target high byte becomes ff. The equal
	// ff80 control ignores that invalid target, while 0080 must fault atomically.
	if err := v.Run(2); err != ErrBudget {
		t.Fatal(err)
	}
	notEqualFault(t, v, ErrInvalidTarget)
	v, err = NewWithSymbols([]byte{0x36, 0, 0x80, 6, 0xff, 0x80, 0x40, 0, 0, 11, 0xff, 0xff}, 0, []SymbolRegion{{ProgramOffset: &offset, Length: 2}})
	if err != nil {
		t.Fatal(err)
	}
	if err := v.Run(3); err != ErrBudget || v.pc != 10 || v.depth != 0 {
		t.Fatalf("live equal: %v pc=%d", err, v.pc)
	}
}
