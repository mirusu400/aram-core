...
func TestClockedRunnerPreservesModeAcrossSlicesAndStops(t *testing.T) {
	...
	check(t, backend.WriteRegister(cpu.RegisterR0, 0x1101))
	clock := &recordingClock{}
...
func TestClockedRunnerDrivesSyntheticQualcommTimeTickIRQ(t *testing.T) {
	...
	check(t, err)
	bus := NewBus()
	check(t, bus.MapRAM("vectors-and-code", 0, 0x2000))
	check(t, bus.MapMMIO(
	...
	check(t, backend.WriteRegister(cpu.RegisterCPSR, 0x13))
...
func writeClockedRunnerARM(
	...
	check(t, backend.WriteMemory(address, code))
}
