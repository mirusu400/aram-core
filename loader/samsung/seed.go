package samsung

import "encoding/binary"

// These two substitution boxes are the S0 and S1 definitions published in
// RFC 4269, Appendix A.1. Keeping the standard source in code makes WBIN
// decoding independent of a phone memory dump.
var seedSBox0 = [256]byte{
	,
}

var seedSBox1 = [256]byte{
	,
}

func seedG(value uint32) uint32 {
	return seedSS0(seedSBox0[byte(value)]) ^
		seedSS1(seedSBox1[byte(value>>8)]) ^
		seedSS2(seedSBox0[byte(value>>16)]) ^
		seedSS3(seedSBox1[byte(value>>24)])
}

func seedSS0(value byte) uint32 {
	x := uint32(value)
	return (x&0x3f)<<24 | (x&0xcf)<<16 | (x&0xf3)<<8 | x&0xfc
}

func seedSS1(value byte) uint32 {
	x := uint32(value)
	return (x&0xfc)<<24 | (x&0x3f)<<16 | (x&0xcf)<<8 | x&0xf3
}

func seedSS2(value byte) uint32 {
	x := uint32(value)
	return (x&0xf3)<<24 | (x&0xfc)<<16 | (x&0x3f)<<8 | x&0xcf
}

func seedSS3(value byte) uint32 {
	x := uint32(value)
	return (x&0xcf)<<24 | (x&0xf3)<<16 | (x&0xfc)<<8 | x&0x3f
}

func seedRound(left0, left1, right0, right1, key0, key1 uint32) (uint32, uint32) {
	temp0 := right0 ^ key0
	temp1 := (right1 ^ key1) ^ temp0
	temp1 = seedG(temp1)
	temp0 = seedG(temp0 + temp1)
	temp1 = seedG(temp1 + temp0)
	temp0 += temp1
	return left0 ^ temp0, left1 ^ temp1
}

// decryptSEEDBlockLE decrypts one WBIN block. The OEM downloader loads both
// ciphertext and its stored round-key schedule as little-endian words.
func decryptSEEDBlockLE(block []byte, roundKeys *[32]uint32) {
	left0 := binary.LittleEndian.Uint32(block[0:4])
	left1 := binary.LittleEndian.Uint32(block[4:8])
	right0 := binary.LittleEndian.Uint32(block[8:12])
	right1 := binary.LittleEndian.Uint32(block[12:16])
	for roundIndex := 15; roundIndex >= 0; roundIndex-- {
		keyIndex := roundIndex * 2
		if roundIndex&1 != 0 {
			left0, left1 = seedRound(
				left0, left1, right0, right1,
				roundKeys[keyIndex], roundKeys[keyIndex+1],
			)
		} else {
			right0, right1 = seedRound(
				right0, right1, left0, left1,
				roundKeys[keyIndex], roundKeys[keyIndex+1],
			)
		}
	}
	binary.LittleEndian.PutUint32(block[0:4], right0)
	binary.LittleEndian.PutUint32(block[4:8], right1)
	binary.LittleEndian.PutUint32(block[8:12], left0)
	binary.LittleEndian.PutUint32(block[12:16], left1)
}
