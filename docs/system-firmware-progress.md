...
- `loader/samsung` identifies WBT, WBIN, DAT, and FNT roles from wrapper
  ...
  for the profiled 512-byte/16 KiB and 2 KiB/128 KiB NAND geometries,
  ...
  are not part of matching. A non-ELF raw WBIN is accepted only after all four
  pieces match an exact profile which declares that opaque format.
...
- `system.QualcommNAND` implements the evidenced 512-byte- and 2 KiB-page
  controller paths, including identification, configuration, ready signaling,
  command/address latches, the 512-byte data window used by the original boot
  stages, and a shareable persistent spare/OOB interface. `system.OneNAND`
  implements the separate 16-bit command, BootRAM/DataRAM, main/spare transfer,
  program, erase, reset, and identification path used by dual-flash boards.
- The named `qualcomm.pbl-hle.nand-v1` and
  `qualcomm.pbl-hle.nand2k-v1` boundaries supply only the unavailable mask-ROM
  service tables and register contracts consumed by QCSBL. Geometry is derived
  from the assembled image; OEMSBL and AMSS are not host-preloaded.
...
## Private raw-download OEMSBL handoff gate

These gates prove only that each exact four-piece download set is normalized,
the profiled PBL ABI starts its original QCSBL, and guest execution reaches the
original OEMSBL entry. They are not cold-boot, display, input, filesystem, or
...
metadata. W270 carries its small-page geometry through the loader, flash image,
board profile, NAND controller, and PBL service tables. Its retained PBL page
