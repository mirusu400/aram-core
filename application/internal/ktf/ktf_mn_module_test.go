...
// The old contiguous walk mistook data after the last class for an unrelocated
// sentinel and dereferenced address 0x508. The actual table bounds exclude that
// data altogether. Preserve this regression with a real sparse-table header.
func TestKTFRegisterMNClassesIgnoresUnreferencedTrailingData(t *testing.T) {
	runtime, err := NewRuntime(interpreter.New(), ktf.Package{
		ClientName: "client.bin0",
	...
	const (
		moduleTable = ImageBase + 0x40
		slots       = ImageBase + 0x80
		object      = ImageBase + 0x100
		record      = object + 4
		sentinel    = ImageBase + 0x200
		sentinelRec = sentinel + 4
		// Values the old walk mistook for a next link and a descriptor.
		// Neither is referenced by the sparse class table.
		outsideImage      = 0x00080000
		unrelocatedOffset = 0x500
	)
	check(t, runtime.writeWords(moduleTable, []uint32{slots, 1, 2}))
	check(t, runtime.writeWords(slots, []uint32{object, 0}))

	fields := []struct {
		address uint32
		value   uint32
	}{
		{object, record}, // self pointer, not a list link
		{object + 4, 0},
		{object + 8, descriptor},   // class descriptor pointer
		{object + 12, 0},           // reserved
		{object + 16, 0},           // flags
		{object + 20, sentinelRec}, // outside the five-word class object
		...
		{sentinel + 12, 0},
		{sentinel + 16, 0},
		{sentinel + 20, outsideImage}, // unrelated trailing data
	}
	...
	if got := runtime.JavaClasses["Real"]; got != object {
		t.Fatalf("registered Real = 0x%08x, want 0x%08x", got, object)
	}
}
