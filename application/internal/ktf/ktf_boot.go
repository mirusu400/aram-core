...
func NewRuntimeForProfile(
	...
	return &Runtime{
		...
		vectorCapacities:         make(map[uint32]uint32),
		vectorCapacityIncrements: make(map[uint32]uint32),
		...
		mediaDefaultVolumes:      make(map[int32]int32),
		...
		inputConstraints:         make(map[uint32]int32),
		inputListeners:           make(map[uint32]uint32),
		inputModes:               make(map[uint32]int32),
		inputSymbolBounds:        make(map[uint32][4]int32),
		...
		stringBufferCaps:         make(map[uint32]uint32),
		...
		printStreamErrors:        make(map[uint32]bool),
		files:                    make(map[uint32]*ktfFile),
		FileData:                 fileData,
		fileStreamTargets:        make(map[uint32]uint32),
		wipi2IODevices:           make(map[uint32]*ktfWIPI2IODevice),
		wipi2SMSMessages:         make(map[uint32][]byte),
		wipi2ResourceGroups:      make(map[uint32]*ktfWIPI2ResourceGroup),
		wipi2Resources:           make(map[string]map[string]*ktfWIPI2Resource),
		wipi2Addresses:           make(map[int]*ktfWIPI2Address),
		wipi2AddressObjects:      make(map[uint32]int),
		wipi2AddressShortcuts:    make(map[int][2]int),
		wipi2NextAddressID:       1,
		wipi2GPSConfigs:          make(map[uint32]ktfWIPI2GPSConfig),
		wipi2GPSLocations:        make(map[uint32]ktfWIPI2GPSLocation),
		wipi2StationLocations:    make(map[uint32]ktfWIPI2StationLocation),
		images:                   make(map[uint32]image.Image),
		animateImages:            make(map[uint32]*ktfAnimateImage),
